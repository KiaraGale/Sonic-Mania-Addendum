...
void ERZStart_Update(void)
{
	...
	if (--self->timer <= 0) {
		...
		SceneInfo->minutes      = RSDK.Rand(0, 10);
	}
...
#endif
	if (Player->superDashCooldown == 0 && leader->state == ERZStart_State_PlayerSuperFly)
		RSDK.SetSpriteAnimation(leader->aniFrames, ANI_RUN, &leader->animator, false, 0);
}
	...
	if (Addendum_GetOptionsRAM()->secondaryGems == SECONDGEMS_SUPEREMERALD) {
		if (SaveGame_GetSaveRAM()->collectedEmeralds == 0b01111111 && Addendum_GetSaveRAM()->collectedSuperEmeralds == 0b01111111) {
			for (int32 e = 0; e < 16; ++e) {
				...
				emerald->originPos.y        = player2->classID == Player->classID ? (player1->position.y + player2->position.y) / 2 : player1->position.y;
...
bool32 ERZStart_Cutscene_FadeIn(EntityCutsceneSeq *host)
{
	...
	if (!host->timer) {
		player1->position.y = player2->classID == Player->classID ? ruby->position.y - 0x20000 : ruby->position.y;
		player1->state      = Player_State_Static;
		player1->stateInput = StateMachine_None;
		player2->position.y = ruby->position.y + 0x20000;
		...
		if (Addendum_GetOptionsRAM()->secondaryGems == SECONDGEMS_SUPEREMERALD) {
			if (SaveGame_GetSaveRAM()->collectedEmeralds == 0b01111111 && Addendum_GetSaveRAM()->collectedSuperEmeralds == 0b01111111) {
				for (int32 i = 0; i < 16; ++i) {
					...
					emerald->originPos.y = player2->classID == Player->classID ? (player1->position.y + player2->position.y) / 2 : player1->position.y;
					emerald->angle       = angle;
					emerald->radius      = 0x2A00;
					emerald->groundVel   = 835;
				...
				for (int32 i = 0; i < 7; ++i) {
					...
					emerald->originPos.x        = player1->position.x;
					emerald->originPos.y =
						player2->classID == Player->classID ? (player1->position.y + player2->position.y) / 2 : player1->position.y;
					emerald->angle     = angle;
	...
	ERZStart_HandlePlayerHover(player1, host, player2->classID == Player->classID ? ruby->startPos.y - 0x9A000 : ruby->startPos.y);
...
bool32 ERZStart_Cutscene_EnterEggman(EntityCutsceneSeq *host)
{
	...
	if (!host->timer) {
		...
		eggman->position.y  = king->originPos.y + 0x9A000;
		eggman->originPos.y = king->originPos.y + 0x9A000;
	...
	camera->position.x = MAX(eggman->originPos.x + 0x9A000, host->storedTimer);
	...
	kingArmL->armBezierPos.x       = king->originPos.x + 0x9A000;
	...
	if (++ERZStart->timer > 96) {
		...
		if (ERZStart->timer >= 32) {
			if (ERZStart->timer == 32) {
				...
				if (player2->classID == Player->classID)
					Player_GiveRings(player2, 50, false);
				// need a special transformation function to have the vanilla transformation sequence rather than the custom Addendum one
				if (addendumOptions->secondaryGems == SECONDGEMS_SUPEREMERALD) {
					Player_TryTransform_ERZ(player1, 0x7F, Addendum_GetSaveRAM()->collectedSuperEmeralds, 0x00);
					if (player2->classID == Player->classID)
						Player_TryTransform_ERZ(player2, 0x7F, Addendum_GetSaveRAM()->collectedSuperEmeralds, 0x00);
				}
				else if (addendumOptions->secondaryGems == SECONDGEMS_TIMESTONE) {
					Player_TryTransform_ERZ(player1, 0x7F, 0x00, Addendum_GetSaveRAM()->collectedTimeStones);
					if (player2->classID == Player->classID)
						Player_TryTransform_ERZ(player2, 0x7F, 0x00, Addendum_GetSaveRAM()->collectedTimeStones);
				}
				else {
					Player_TryTransform_ERZ(player1, 0x7F, 0x00, 0x00);
					if (player2->classID == Player->classID)
						Player_TryTransform_ERZ(player2, 0x7F, 0x00, 0x00);
				}
			}
		}
		else {
...
bool32 ERZStart_Cutscene_StartFight(EntityCutsceneSeq *host)
{
	...
	if (player1->animator.animationID != 56) {
		Zone->playerBoundActiveR[0] = true;
	...
	if (player2->classID == Player->classID) {
		if (player2->animator.animationID != 56) {
			player2->state            = ERZStart_State_PlayerSuperFly;
			player2->abilityValues[0] = 60;
			player2->velocity.x       = 0x160000;
			player2->onGround         = false;
			player2->stateAbility     = ERZStart_Player_StartSuperFly;
			player2->stateInput       = Player_Input_P2_AI;

			if (player2->characterID == ID_KNUCKLES) {
				RSDK.SetSpriteAnimation(player2->aniFrames, ANI_GLIDE, &player2->animator, false, 6);
				player2->animator.rotationStyle = ROTSTYLE_FULL;
			}
			else if (player2->characterID == ID_MIGHTY) {
				RSDK.SetSpriteAnimation(player2->aniFrames, 57, &player2->animator, false, 1);
			}
			else {
...
bool32 ERZStart_Cutscene_Fight(EntityCutsceneSeq *host)
{
	...
	if (player1->superState == SUPERSTATE_SUPER) {
		...
		player2->deathType = PLAYER_DEATH_DIE_USESFX;
	}

	if (eggman->health <= 2 && king->health <= 2 && !ERZStart->pinchMusic) {
		Music_TransitionTrack(TRACK_ERZBOSS, 0.125);
		ERZStart->pinchMusic = true;
	}
...
void ERZStart_Player_HandleSuperDash(EntityPlayer *player)
{
	...
	else if (player->characterID == ID_MIGHTY)
		RSDK.SetSpriteAnimation(player->aniFrames, 57, &player->animator, false, 1);
	else
...
void ERZStart_State_PlayerSuperFly(void)
{
	foreach_active(Player, player)
	{
		if (player->onGround) {
			...
			if (velY >= -player->topSpeed) {
				if (velY > player->topSpeed)
					player->velocity.y = velY - (player->acceleration >> 1) - player->acceleration;
			}
			else {
				player->velocity.y = player->acceleration + velY + (player->acceleration >> 1);
			}

			if (player->left) {
				if (player->characterID == ID_KNUCKLES && player->direction == FLIP_NONE)
					RSDK.SetSpriteAnimation(player->aniFrames, ANI_GLIDE, &player->animator, true, 0);
				...
				if (player->characterID == ID_KNUCKLES && player->direction == FLIP_X)
					RSDK.SetSpriteAnimation(player->aniFrames, ANI_GLIDE, &player->animator, true, 0);
			...
			if (velX >= -player->topSpeed) {
				if (velX > player->topSpeed)
					player->velocity.x = velX - (player->acceleration >> 1) - player->acceleration;
			}
			else {
				player->velocity.x = player->acceleration + velX + (player->acceleration >> 1);
			}
...
void ERZStart_Player_StartSuperFly(void)
{
	...
	if (self->jumpAbilityState == 1 && self->jumpPress) {
		...
		else if (self->characterID == ID_MIGHTY) {
			RSDK.SetSpriteAnimation(self->aniFrames, 57, &self->animator, false, 1);
		}
		else {
...
void ERZStart_State_PlayerRebound(void)
{
	...
	else if (entity->characterID == ID_MIGHTY) {
		RSDK.SetSpriteAnimation(entity->aniFrames, 57, &entity->animator, false, 1);
	}
	else {
