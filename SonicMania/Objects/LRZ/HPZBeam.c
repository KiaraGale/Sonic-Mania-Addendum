...
void HPZBeam_Update(void) {
	...
	if (!self->hit_timer) {
		...
		if (self->target_y <= self->position.y) {
			...
			// FUCKING STUPID
			for (int32 i = 0; i < 7; ++i) {
				EntityHPZEmerald *emerald = HPZIntro->SortedSuperEmeralds[i];
				if (emerald && emerald->position.x == self->position.x && emerald->position.y > self->position.y - TO_FIXED(16)) {
					...
					RSDK.SetSpriteAnimation(HPZEmerald->aniFrames, 4, &debris->animator, true, 0);
					...
					debris = CREATE_ENTITY(Debris, Debris_State_FallAndFlicker, emerald->position.x, emerald->position.y);
					RSDK.SetSpriteAnimation(HPZEmerald->aniFrames, 4, &debris->animator, true, 1);
					debris->drawGroup       = Zone->objectDrawGroup[1];
					debris->velocity.x      = 0x20000;
					debris->velocity.y      = -0x40000;
					debris->gravityStrength = 0x4800;
					...
					debris = CREATE_ENTITY(Debris, Debris_State_FallAndFlicker, emerald->position.x, emerald->position.y);
					RSDK.SetSpriteAnimation(HPZEmerald->aniFrames, 4, &debris->animator, true, 2);
					debris->drawGroup       = Zone->objectDrawGroup[1];
					...
					debris = CREATE_ENTITY(Debris, Debris_State_FallAndFlicker, emerald->position.x, emerald->position.y);
					RSDK.SetSpriteAnimation(HPZEmerald->aniFrames, 4, &debris->animator, true, 3);
					debris->drawGroup       = Zone->objectDrawGroup[1];
					debris->velocity.x      = 0x10000;
					debris->velocity.y      = -0x20000;
					debris->gravityStrength = 0x4800;
...
void HPZBeam_StageLoad(void) {
	...
	HPZBeam->aniFrames = RSDK.LoadSpriteAnimation("LRZ3/WarpBeam.bin", SCOPE_STAGE);
}
